...
app.get('/allFonts', function(req, res) { 
	...
	req.session.allowedFormats = [];
		...
		for(var i = 0 ; i < lines.length ; i++){
			  ...
			  if(query == 'pictures')req.session.todolist.push("pictures/"+lines[i]);
		...
		res.render('allPictures.ejs', {todolist: req.session.todolist});
...
app.post('/upload', function(req, res) {
	...
	if( isValidType(type) ){
		...
		sampleFile.mv( folderName +"/"+ replaceEmptySpace , function(err) {
        if (err) {
